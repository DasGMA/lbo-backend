...
router.route("/post-business").post(protected, async (req, res) => {
  ...
  if (accountType === "business" || accountType === "admin") {
    try {
      ...
      await BusinessImage.findByIdAndUpdate(
        { _id: businessImages._id },
        { $set: { postedBy: savedBusiness._id } },
      );
    ...
    res
      .status(400)
      .json({ Message: "You have no priviledges to post a business." });
  }
});
...
router.route("/edit-business").post(protected, async (req, res) => {
  ...
  if (
    accountType === "admin" ||
    (accountType === "business" && postedBy === postedByID)
  ) {
    try {
